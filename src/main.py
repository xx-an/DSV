...
CHECK_RESULTS = ['sound', 'unsound', 'unresolved']
...
def set_logger(disasm_path, disasm_type):
    for log_name in utils.LOG_NAMES:
        logger_path = disasm_path.replace('.' + disasm_type, '.' + log_name)
        utils.setup_logger(log_name, logger_path, logging.INFO)
...
def dsv_main(exec_path, disasm_path, disasm_type):
    set_logger(disasm_path, disasm_type)
    global_var.get_elf_info(exec_path)
...
def dsv_batch(elf_lib_dir, disasm_lib_dir, disasm_type):
...
if __name__=='__main__':
    ...
    parser.add_argument('-d', '--disassembler', default='objdump', type=str, help='Disassembler')
    ...
    parser.add_argument('-p', '--disasm_path', default='micro-benchmarks', type=str, help='Benchmark library') 
    parser.add_argument('-e', '--elf_path', default='micro-benchmarks', type=str, help='Elf shared object library') 
    parser.add_argument('-f', '--file_name', type=str, help='Benchmark file name')
    args = parser.parse_args()
    disasm_type = args.disassembler
    disasm_lib_dir = os.path.join(utils.PROJECT_DIR, args.disasm_path)
    ...
    if args.soundness:
        ...
        if args.batch:
            dsv_batch(elf_lib_dir, disasm_lib_dir, disasm_type)
        else:
