...
class Disasm_Hopper(Disasm):
    ...
    def _read_variable_value(self, line):
        if line.startswith(';') and ': ' in line and ',' in line:
            ...
            var_value = var_split[1].strip().split(',', 1)[-1].strip()
    ...
    def _replace_symbol(self, symbol):
        ...
        if not (utils.imm_start_pat.match(symbol) or symbol in lib.REG_NAMES):
            if symbol in self.label_address_map:
                ...
                remaining = symbol.split('_', 1)[1].strip()
                if utils.imm_pat.match(remaining):
                    res = hex(utils.imm_str_to_int(remaining))
            # else:
            #     print(symbol)
        return res
        
    def _replace_to_times_elements(self, content):
        res = content
        c_split = content.split('*')
        for i, c in enumerate(c_split):
            c_split[i] = self._replace_symbol(c)
        res = '*'.join(c_split)
        return res

    def _replace_each_symbol(self, content):
        res = content
        if '+' in content:
            c_split = content.split('+')
            for idx, elem in enumerate(c_split):
                if '*' in elem:
                    c_split[idx] = self._replace_to_times_elements(elem)
                else:
                    c_split[idx] = self._replace_symbol(elem)
            res = '+'.join(c_split)
        elif '-' in content:
            c_split = content.split('-')
            for idx, elem in enumerate(c_split):
            ...
            res = self._replace_to_times_elements(content)
        else:
            res = self._replace_symbol(content)
        return res

    def _replace_symbol_with_value(self, arg):
        res = arg
        if arg.endswith(']'):
            arg_split = arg.split('[', 1)
            prefix = arg_split[0].strip()
            mem_addr = arg_split[1].strip().rsplit(']', 1)[0].strip()
            mem_addr = self._replace_each_symbol(mem_addr)
            res = prefix + ' [' + mem_addr + ']'
        else:
            res = self._replace_each_symbol(arg)
        return res
    ...
    def _format_arg(self, address, inst_name, arg, rip):
        res = self._replace_symbol_with_value(arg)
        res = helper.add_ptr_suffix_arg(res)
        res = res.replace('+-', '-')
